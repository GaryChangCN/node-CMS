  ...
  <div id="sale">
    <div class="secondTitle">
      ...
      <div>
        ...
        <el-input placeholder="搜索表单" icon="search" v-model="keywords" v-show="search.inputShow" @keyup.native.enter="searchSale" @click="searchSale"></el-input>
      </div>
    </div>
    <div class="table">
      ...
      <el-form :model="moveForm" :rules="moveRule" ref="moveRef">
        <el-form-item label="请选择" prop="new">
          <el-select v-model="moveForm.new">
            <el-option label="发货清单" value="delivery"></el-option>
            <el-option label="回收站" value="recycle" disabled></el-option>
          </el-select>
        </el-form-item>
      </el-form>
...
<script>
    ...
    export default {
        ...
        mounted() {
            ...
            var _this = this;
            this.axios.get(CONFIG.API + 'order/count/sale').then(function(value) {
                var json = value.data;
        ...
        methods: {
            ...
            searchSale() {
                var _this = this;
                this.processBarBegin();
                this.axios.get(CONFIG.API + 'order/search/sale/' + this.keywords).then(function(value) {
                    var json = value.data;
                    if (!json.err && json.state) {
            ...
            getList(skip) {
                var _this = this;
                this.processBarBegin();
                this.axios.get(CONFIG.API + 'order/get/sale/15/' + skip).then(function(value) {
                    var json = value.data;
                    if (!json.err && json.state) {
                        _this.tableData = _this.fullfill(json.list);
                        _this.processBarEnd();
