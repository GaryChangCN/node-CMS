  ...
  <div id="processed">
    <div class="secondTitle">
      ...
      <div>
        ...
        <el-input placeholder="搜索表单" icon="search" v-model="keywords" v-show="search.inputShow" @keyup.native.enter="searchProcessed" @click="searchProcessed"></el-input>
      </div>
    </div>
    <div class="table">
      ...
      <el-form :model="moveForm" :rules="moveRule" ref="moveRef">
        <el-form-item label="请选择" prop="new">
          <el-select v-model="moveForm.new">
            <el-option label="销售发票" value="sale"></el-option>
            <el-option label="未成功交易" value="failed"></el-option>
          </el-select>
        </el-form-item>
      </el-form>
...
<script>
    ...
    export default {
        ...
        mounted() {
            ...
            var _this = this;
            this.axios.get(CONFIG.API + 'order/count/processed').then(function(value) {
                var json = value.data;
        ...
        methods: {
            ...
            searchProcessed() {
                var _this = this;
                this.processBarBegin();
                this.axios.get(CONFIG.API + 'order/search/processed/' + this.keywords).then(function(value) {
                    var json = value.data;
                    if (!json.err && json.state) {
            ...
            getList(skip) {
                var _this = this;
                this.processBarBegin();
                this.axios.get(CONFIG.API + 'order/get/processed/15/' + skip).then(function(value) {
                    var json = value.data;
                    if (!json.err && json.state) {
                        _this.tableData = _this.fullfill(json.list);
                        _this.processBarEnd();
